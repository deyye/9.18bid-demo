...
const OutlineEdit: React.FC<OutlineEditProps> = ({ onNext }) => {
    ...
    // 🟢 新增：递归重新编号整个大纲 (核心逻辑)
    // parentId: 父节点的ID (如 "1.2")，如果是根节点则为空
    // level: 当前层级
    const renumberOutline = (items: OutlineItem[], parentId: string = '', level: number = 1): OutlineItem[] => {
        return items.map((item, index) => {
            // 生成新 ID：如果有父ID，则拼接；否则直接用序号
            ...
            return {
                ...
                level: level,
                // 递归处理子节点
                children: item.children ? renumberOutline(item.children, newId, level + 1) : []
            };
        });
    };
    ...
    const deleteNodeRecursively = (items: OutlineItem[], id: string): OutlineItem[] => {
        return items
            ...
            .map(item => {
                if (item.children) {
                    return { ...item, children: deleteNodeRecursively(item.children, id) };
                }
                return item;
            });
    };
    ...
    const handleDistributeWords = () => {
        ...
        message.success(`已分配，每章约 ${perNodeCount} 字`);
    };
    ...
    const handleModalOk = () => {
        ...
        let newOutline = [...state.outline];

        if (modalMode === 'edit' && currentEditNode) {
            // 编辑标题：不需要重编号
            newOutline = updateTitleRecursively(newOutline, currentEditNode.id, modalInputValue);
            message.success("修改成功");
        } else if (modalMode === 'add' && currentEditNode) {
            // 添加节点：需要重编号
            const newItem: OutlineItem = {
                id: "temp_id", // 临时ID，稍后会被 renumberOutline 覆盖
                title: modalInputValue,
                level: currentEditNode.level + 1,
                children: []
            };
            // 1. 添加
            newOutline = addChildRecursively(newOutline, currentEditNode.id, newItem);
            // 2. 重编号 (核心修正)
            newOutline = renumberOutline(newOutline);
    ...
    const handleDeleteNode = (id: string) => {
        // 1. 删除
        let newOutline = deleteNodeRecursively(state.outline, id);
        // 2. 重编号 (核心修正)
        newOutline = renumberOutline(newOutline);
        
        setState({ outline: newOutline });
        message.success("删除成功，章节号已自动更新");
    };
    ...
    const onDrop: TreeProps['onDrop'] = (info) => {
        ...
        let data = JSON.parse(JSON.stringify(state.outline)); // 深拷贝防止引用问题
    ...
    const handleGenerateContent = () => {
        ...
        generateContentStream(
            state.documentContent, state.overview, state.requirements, state.outline, state.config,
            (chapterId, content) => {
                setState((prev: AppState) => ({
                    ...prev,
                    generatedContent: { ...prev.generatedContent, [chapterId]: content }
                }));
            }
        ).then(() => {
            ...
            console.error(error);
            setState({ isGenerating: false });
            message.error('生成过程中断');
        });

        Modal.success({
            title: '🚀 内容生成已启动',
            content: (<div><p>AI 正在后台并行生成所有章节内容。</p><p>您可以立即前往<b>编辑页面</b>，内容将实时逐章呈现。</p></div>),
            okText: '立即前往编辑',
            onOk: () => { onNext(); },
            cancelText: '稍后',
            closable: true,
                                ...
                                <Col flex="auto">
                                    <Space size="large">
                                        ...
                                        <InputNumber 
                                            ...
                                            addonAfter="字"
                                        />
                                        <Button type="primary" icon={<CalculatorOutlined />} onClick={handleDistributeWords}>一键智能分配</Button>
                                    </Space>
                                </Col>
                                <Col><Tag color="orange">支持拖拽调整结构</Tag></Col>
                            </Row>
                        </div>
